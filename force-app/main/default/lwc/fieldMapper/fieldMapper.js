...
export default class FieldMapper extends LightningElement {
  ...
  get isEditMode() {
    return !!this.draftMapping.id;
  }
  get formTitle() {
    return this.isEditMode ? "Edit Mapping" : "New Mapping";
  }

  get isArrayMapping() {
    return this.draftMapping.isArray;
  }
  get scalarTabClass() {
    return !this.draftMapping.isArray ? "toggle-tab active" : "toggle-tab";
  }
  get arrayTabClass() {
    return this.draftMapping.isArray ? "toggle-tab active" : "toggle-tab";
  }
  ...
  get dataTypeOptions() {
    ...
    return types.map((t) => ({
      label: t,
      value: t,
      selected: this.draftMapping.dataType === t
    }));
  }
  ...
  syncEnrichedOptions() {
    ...
    this.enrichedSfFieldOptions = sourceList.map((opt) => {
      const existing = this.enrichedSfFieldOptions.find(
        (e) => e.value === opt.value
      );
      return {
        ...opt,
        ...
        targetObjectLabel: existing
          ? existing.targetObjectLabel
          : "Target Object"
      };
    });
  }

  get sfFieldDisplayValue() {
    return this.draftMapping.sourceField || "Select Salesforce Field";
  }
  get sfFieldDisplayClass() {
    return this.draftMapping.sourceField
    ...
    this.enrichedSfFieldOptions = this.enrichedSfFieldOptions.map((opt) => {
      if (opt.value === fieldValue) {
        ...
        return {
          ...opt,
          isExpanded: true,
          isLoadingTarget: !alreadyLoaded,
          targetObjectLabel: alreadyLoaded
            ? opt.targetObjectLabel
            : "Loading..."
        };
      }
        ...
        this.enrichedSfFieldOptions = this.enrichedSfFieldOptions.map((o) => {
          if (o.value === fieldName) {
            return {
              ...
              targetFields: res,
              targetObjectLabel: "Related Fields"
            };
          }
          return o;
        });
  ...
  handleMappingChange(e) {
    if (e.target.type === "checkbox")
      this.draftMapping[e.target.name] = e.target.checked;
  ...
  handleSaveMapping() {
    if (
      !this.draftMapping.developerName ||
      !this.draftMapping.sourceField ||
      !this.draftMapping.targetField
    ) {
