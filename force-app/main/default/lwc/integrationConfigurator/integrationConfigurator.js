...
export default class IntegrationConfigurator extends LightningElement {
    ...
    get isViewList() { return this.currentView === 1; }
    get isViewForm() { return this.currentView === 2; }
    get isViewMappings() { return this.currentView === 3; }
    get isEmptyState() { return this.configurations.length === 0; }
    get showBackButton() { return this.currentView > 1; }
    get showNewButton() { return this.currentView === 1; }
    get isEditMode() { return !!this.draftConfig.id; }
    get formTitle() { return this.isEditMode ? 'Edit Configuration' : 'New Configuration'; }

    // Draft properties
    get isOutbound() { return this.draftConfig.direction === 'Outbound'; }
    get isInbound() { return this.draftConfig.direction === 'Inbound'; }
    get isBoth() { return this.draftConfig.direction === 'Both'; }
    ...
    get sfObjectDisplayClass() {
        return this.draftConfig.sourceObject ? 'field-picker-display-text' : 'field-picker-placeholder-text';
    }
    get filteredSfObjectOptions() {
        ...
        return this.sfObjectOptions.filter(o => o.label.toLowerCase().includes(term));
    }
    ...
    handlePickerSearch(e) { this.pickerSearchTerm = e.target.value; }
    handleSfObjectSelect(e) {
        this.draftConfig.sourceObject = e.currentTarget.dataset.value;
        this.isSfObjectPickerOpen = false;
    }
    handleInnerClick() { this._isInnerClick = true; }

    renderedCallback() {
        if (!this._clickBound) {
            this._clickBound = true;
            document.addEventListener('mousedown', () => {
                if (this._isInnerClick) { this._isInnerClick = false; return; }
                this.isSfObjectPickerOpen = false;
            });
        }
    }
    ...
    handleCancelConfig() { this.currentView = 1; }
    
    handleSaveConfig() {
        if (!this.draftConfig.developerName || !this.draftConfig.sourceObject || !this.draftConfig.externalSystem) {
